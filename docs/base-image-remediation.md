...
The build also refreshes available Debian security updates and removes the package-list cache in the same layer. This is necessary because the initial published Trixie base still contained packages for which Trivy reported fixed security updates. The image scan remains a blocking gate, so future high- or critical-severity findings are not ignored.
